...
def generate_qrcode(event, size, type):
    """
    ...
    qr.add_data(f"https://rbreg.vercel.app/events/{type}/{event.get('uid')}")
    qr.make(fit=True)
    ...
    if size == "large" and type == "register":
        template = Image.open("static/assets/rr_qr_template_large_register.png")
    elif size == "small" and type == "checkin":
        template = Image.open("static/assets/rr_qr_template_checkin.png")
    elif size == "small" and type == "register":
        template = Image.open("static/assets/rr_qr_template_register.png")
    else:
    ...
    # Add URL
    text = f"https://roboregistry.vercel.app/events/{type}/{event.get('uid')}"
    ...
    if type == "register":
        ...
        text_width, text_height = draw.textsize(text, font)
        draw.text(((template_width - text_width) // 2, template_height - text_height - 600), text, (0, 0, 0), font=font)
