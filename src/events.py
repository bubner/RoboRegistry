...
def redirector():
    ...
    if request.method == "POST":
        ...
        if not (event := db.get_event(target)):
            ...
            if not target.startswith("https://roboregistry.vercel.app") or not target.startswith("https://rbreg.vercel.app") or res.status_code == 404:
...
def ci():
    ...
    if request.method == "POST":
        ...
        if not link or not link.startswith("https://roboregistry.vercel.app") or not link.startswith("https://rbreg.vercel.app"):
